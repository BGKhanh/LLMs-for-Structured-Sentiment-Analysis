...
class Llama3Model(BaseModel):
    """
    Llama 3/3.2 model implementation.
    
    Handles:
    - Llama 3 specific loading (AutoModelForCausalLM)
    - Standard chat template application (system/user roles)
    - Batch inference with precise token tracking
    - JSON response extraction
    
    Reference: meta-llama/Llama-3.2-3B-Instruct
    """
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            # Configure pad_token if missing (Crucial for Llama 3 batching)
            ...
            raise RuntimeError(f"Failed to load Llama 3 model: {str(e)}")
    ...
    def extract_response(self, raw_response: str) -> str:
        """
        Extract JSON from Llama 3's response.
        """
